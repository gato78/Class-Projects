...
def applyingDijkstraAlgorithm(start_node, goal_node):
	...
	while len(ListOfNodes.Node_State_i) > 0:
		...
		if currNode == goal_node:		# Should be replaced by: if rigid_robot_goal_space(x,y,goal_x,goal_y):
			break
...
def pygame_animation(resize, obs_lst,djik_path,v_nodes):
	...
	while RUN_PROGRAM:
		...
		pyg.time.wait(6000)		# freeze display window for 8 seconds to show path to Goal Node
		RUN_PROGRAM = False
	pyg.quit()
